...
__all__ = ['MyCOCOInstance']


class MyCOCOInstance(VisionDataset):
    ...
    def __init__(self, root=os.path.join('~', '.mxnet', 'datasets', 'coco'),
                 ...
                 skip_empty=True):
        super(MyCOCOInstance, self).__init__(root)
    ...
    def _load_jsons(self):
        ...
        for split in self._splits:
            anno = os.path.join(self._root, split) + '.json'
            _coco = COCO(anno)
            ...
            for entry in _coco.loadImgs(image_ids):
                dirname, filename = entry['file_name'].split('/')[-2:]
